# CROSSUB Landlord App — CLAUDE.md

The CROSSUB landlord mobile-web app (Next.js 16; the app is in `apps/landlord/`). One of
five role apps; the others and the backend are **sibling repos** under
`~/Desktop/crossub/`.
...
- Typed client: `apps/landlord/lib/crossub-api/client.ts` — `createCrossubClient`
  (openapi-fetch, base `/api/v1`, cookie session via `credentials: 'include'`).
- This app's facade is **`/api/v1/landlord/*`** (read-only) — owned properties, their
  maintenance + inspections, and owner payout statements. Add calls in
  `apps/landlord/lib/crossub-api/landlord-client.ts` using the `crossub` client +
  `components['schemas'][...]` types. **Never hand-roll request/response types.**

## Where things live (sibling repos under `~/Desktop/crossub/`)

- **Backend (NestJS):** `crossub_web/apps/api` — the landlord facade is `/api/v1/landlord/*`.
...
- Cookie session (`csb_at`) via `/auth/login`; every call goes through the BFF proxy
  `apps/landlord/app/api/[...path]/route.ts` → `API_INTERNAL_URL`.
